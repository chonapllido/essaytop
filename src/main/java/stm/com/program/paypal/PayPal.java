...
public class PayPal  {
	...
	*/
        ...
        String strSandbox = "";
        if(EgovProperties.getProperty("Globals.paypalSandboxFlag").equals("true")){
        	strSandbox="Sandbox";
        }
      //ButtonSource Tracker Code
        gvBNCode	= EgovProperties.getProperty("Globals.paypalSbnCode");	
    	
        gvAPIUserName	= EgovProperties.getProperty("Globals.paypalUser"+strSandbox);
        gvAPIPassword	= EgovProperties.getProperty("Globals.paypalPassword"+strSandbox);
        gvAPISignature = EgovProperties.getProperty("Globals.paypalSignature"+strSandbox);

        gvAPIEndpoint = EgovProperties.getProperty("Globals.paypalNvpEndpoint"+strSandbox);
        paypalUrl = EgovProperties.getProperty("Globals.paypalCheckoutUrl"+strSandbox);

        gvVersion	= EgovProperties.getProperty("Globals.paypalApiVersion");
        java.lang.System.setProperty("https.protocols", EgovProperties.getProperty("Globals.paypalSslVersionToUse"));
    /*    
	} catch (IOException e) {
...
public Map<String, String> callShortcutExpressCheckout( HttpServletRequest request, final String returnURL, final String cancelURL)
{
	...
	if(isSet(request.getParameter("BRANDNAME") )){
		nvpstr += "&BRANDNAME=" + request.getParameter("BRANDNAME");
	}
