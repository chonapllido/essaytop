...
<div id="contents">
    <div id="content-area" class="form-request-content-revision">
		...
		<div class="form-holder-revision">
			<form:form name="Form_revision" commandName="orderRevisionCommand" method="post" action="${url }" enctype="multipart/form-data" onsubmit="return doSubmit(this, 'OrderRevisionCommand')">
			<input type="hidden" name="order_id" value="${orderRevisionCommand.order_id }" />
			<input type="hidden" name="total_payment" value="${cmd.total_payment }" />
			<input type="hidden" name="total" value=""/>
			<input type="hidden" name="price" value=""/>
			<input type="hidden" name="no_pages" value="${orderRevisionCommand.no_pages }"/>
			<input type="hidden" name="deldaterev" id="deldate" value="<fmt:formatDate pattern="MMM dd" value="${deadline }" /> 7pm (GMT +9 Asian Time)"/>
			<input type="hidden" name="end_date" id="end_date" value="${orderRevisionCommand.end_date }"/>
			<input type="hidden" name="rate" value="${prices }" />
			<input type="hidden" name="hasorder" value="${hasorder }" />
			<input type="hidden" name="pricewithcommission" value="${orderRevisionCommand.pricewithcommission } "/>
     		...
     		<div class="form-col">                                      
				<span class="details">
				    ...
				    <span class="info-help">
				        <input type="text" name="topic" id="topic" value="<c:out value="${orderRevisionCommand.topic }" />" maxlength="100" autocomplete="off" onkeydown="return trapEnter(event,'JSRevision.validateForm()');" title="Main Topic"/>
				    </span>
				</span>                 
				<span class="details">
				    <label>Level</label>
				    <span class="info-help select-arrow-bg">
				        <select name="level" id="level" autocomplete="off" title="Level"/>
				       		<c:forEach var="level" items="${level_list.list}" varStatus="i">
							<option value="${level.title }" <c:if test="${orderRevisionCommand.level eq level.title }">selected="selected"</c:if>>${level.title}</option>
							</c:forEach> 
				        </select>
				    </span>
				</span> 
				<span class="details">
				    ...
				    <span class="info-help select-arrow-bg">
				        <select id="deadline" name="days" autocomplete="off" title="Deadline" onchange="JSRevision.countWords('${words}'); JSRevision.set_date('${fn:substring(orderRevisionCommand.start_date,0,10)}');" />
	                   		<c:forEach var="i" begin="${min_days}" end="30">
							<option value="${i}" <c:if test="${orderRevisionCommand.days eq i }">selected="selected"</c:if>>${i*24} hrs (${i} days)</option>
							</c:forEach> 
	               		</select>
				   	</span>
				...
				<span class="details word-count-details">
				    <span class="info-help no-label">
				        <textarea name="content" id="pasteinfo" autocomplete="off" rows="10" cols="10">${content }</textarea>
					</span>
              	...
              	<span class="details file-list">
                  	...
                  	<input type="file" name="upload" id="upload" title="Files" />
				</span>
				<span class="btns-sec">
				    <span class="total-amt-hldr">
				        <img src="${img_src }/stm/tit/tit_total.png" alt="Total"/><br/>
				        <span class="total-amt" id="total-amt">
				            <img src="${img_src }/stm/tit/tit_dollar_sign.png" alt="$" class="dollar-sign"/><span id="total_text">${orderRevisionCommand.pricewithcommission }</span>
				        </span>                                
				    </span>
